...
const Header = () => {
    ...
    return (
        <nav className='sticky top-0 z-40 text-sm bg-white flex flex-col md:flex-row md:items-center md:justify-between p-5 py-4 gap-4 shadow-xl'>
            ...
            <div className={`flex flex-col md:flex-row md:items-center ${isOpen ? 'block' : 'hidden md:flex'}`}>
                ...
                <ul className='flex flex-col md:flex-row md:gap-4 md:pr-6'>
                    {navLinks.map(({ path, label }) => (
                        <li key={path} className='hover:bg-white shadow-sm transition-all duration-300 p-2 rounded-lg'>
                            <NavLink
                                to={path}
                                onClick={closeMenu}
